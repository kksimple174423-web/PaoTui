...
public class UserController {
    ...
    public APIResponse login(User user){
        ...
        if(responseUser != null) {
            responseUser.setToken(newToken(responseUser));
    ...
    public APIResponse userLogin(User user){
        ...
        if(responseUser != null) {
            if("1".equals(responseUser.getStatus())) {
                return new APIResponse(ResponeCode.FAIL, "账号已被禁用");
            }
            responseUser.setToken(newToken(responseUser));
            userService.updateUser(responseUser);
            return new APIResponse(ResponeCode.SUCCESS, "登录成功", responseUser);
    ...
    private String newToken(User user) {
        return DigestUtils.md5DigestAsHex((user.getUsername() + System.currentTimeMillis() + salt).getBytes());
    }
