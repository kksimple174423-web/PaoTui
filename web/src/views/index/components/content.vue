  ...
  <div class="content">
    ...
    <div class="content-right">
      ...
      <a-spin :spinning="contentData.loading" style="min-height: 200px;">
        <div class="pc-task-list flex-view">
          <div class="task-item" v-for="item in contentData.pageData" :key="item.id" @click="handleDetail(item)">
            <div class="img-view">
              <img :src="item.cover" alt="">
            </div>
            <div class="info-view">
              <div class="task-name">{{ item.title }}</div>
              <div class="price">¥ {{ item.reward }} <span class="reward-label">悬赏</span></div>
              <div class="translators">{{ item.pickupAddress }} → {{ item.deliveryAddress }}</div>
              <div class="translators">
                <span>{{ item.classificationTitle }}</span>
                <span class="status-tag">{{ statusText(item.status) }}</span>
              </div>
            </div>
          </div>
          <div class="no-data" v-if="contentData.pageData.length === 0 && !contentData.loading">暂无任务</div>
        </div>
      </a-spin>
...
import {listApi as listTaskList} from '/@/api/task'
...
const clickTag = (index) => {
  ...
  getTaskList({tag: contentData.selectTagId})
}
...
const selectTab = (index) => {
  ...
  let sort = (index === 0 ? 'recent' : index === 1 ? 'hot' : 'reward')
...
const changePage = (page) => {
  ...
  contentData.pageData = contentData.taskData.slice(start, start + contentData.pageSize)
...
const getTaskList = (data) => {
  ...
  listTaskList(data).then(res => {
    ...
    console.log(res)
    contentData.taskData = res.data
    contentData.total = contentData.taskData.length
    changePage(1)
  }).catch(err => {
    console.log(err)
    contentData.loading = false
