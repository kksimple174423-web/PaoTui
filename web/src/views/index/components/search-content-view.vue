    ...
    <div class="content-list">
      <div class="task-list">
        ...
        <a-spin :spinning="tData.loading" style="min-height: 200px;">
          <div class="tasks flex-view">
            <div class="task-item item-column-4" v-for="item in tData.pageData" @click="handleDetail(item)">
              <div class="img-view">
                <img :src="item.cover"></div>
              <div class="info-view">
                <h3 class="task-name">{{ item.title.substring(0, 12) }}</h3>
                <span>
...
const search = () => {
  ...
  getTaskList({'keyword': tData.keyword})
}
...
const changePage = (page) => {
  ...
  tData.pageData = tData.taskData.slice(start, start + tData.pageSize)
...
const getTaskList = (data) => {
  ...
  listTaskList(data).then(res => {
    ...
    tData.taskData = res.data
    tData.total = tData.taskData.length
    changePage(1)
    tData.loading = false
  }).catch(err => {
    console.log(err)
